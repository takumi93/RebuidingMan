...
public class PartsDatabase : ScriptableObject
{
    ...
    private Dictionary<string, PartsData> allPartsDict;

    // データの初期化
    public void Init()
    {
        // partName をキーにして Dictionary 化
        allPartsDict = allParts.ToDictionary(p => p.partName, p => p);
    }

    // データの検索
    public PartsData GetPartByName(string name)
    {
        ...
        if (allPartsDict.TryGetValue(name, out var part))
        {
            // 見つかった場合
            return part;
        }
